...
class ModelTurnoverWithEQBuilder(StrategyBuilder):
    """Build a turnover-filtered strategy with equal-weight allocation."""
    ...
    def build(self) -> Strategy:
        return Strategy(
            ...
            description=f"Min Turnover {self.MIN_TURNOVER_RUB} for last MIN_TURNOVER_RUB. Allocation is Equal Weighted",
            pipeline=Pipeline(
                steps=[
                    (
...
class ModelHighTurnoverWithEQBuilder(StrategyBuilder):
    """Build a High turnover-filtered strategy with equal-weight allocation."""
